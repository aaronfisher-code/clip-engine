...
const destination = join(process.cwd(), "src-tauri", "binaries");
mkdirSync(destination, { recursive: true });

function findExecutable(name) {
  const explicit = process.env[`CLIP_ENGINE_${name.toUpperCase()}`];
  const candidates = explicit ? [explicit] : (process.env.PATH || "").split(delimiter).map((entry) => join(entry, `${name}${extension}`));
