...
fn initial_video_context() -> VideoContextSettings {
    ...
    let width = display.map(|display| display.width).unwrap_or(1_920);
    let height = display.map(|display| display.height).unwrap_or(1_080);
    let fps = display
...
impl ObsBackend {
    ...
    fn rebuild_graph(&mut self, requested_config: &RecorderConfig) -> Result<()> {
        ...
        let requested_video_context = VideoContextSettings {
            width: resolved.output_width,
            height: resolved.output_height,
            fps: resolved.fps,
        };
...
#[cfg(target_os = "linux")]
fn linked_libobs_owns_runtime_paths(root: &Path) -> bool {
    let Some(linked_libobs_path) = env::var_os("LIBOBS_PATH") else {
        return false;
    };
